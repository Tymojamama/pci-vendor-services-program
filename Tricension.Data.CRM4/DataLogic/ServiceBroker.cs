...
namespace Tricension.Data.CRM4.DataLogic
{
    internal class ServiceBroker
    {
        ...
        internal static CrmService GetServiceInstance(Model.IAuthenticationRequest authRequest)
        {
            ...
            if (!String.IsNullOrEmpty(serviceWrapper.CrmService.CrmAuthenticationTokenValue.CrmTicket) && !_crmServices.ContainsKey(serviceWrapper.CrmService.CrmAuthenticationTokenValue.CrmTicket))
            {
        ...
        private static CrmService CreateServiceInstance(Model.IAuthenticationRequest authRequest)
        {
            ...
            using (TokenBroker tokenBroker = new TokenBroker())
            {
                ...
                serviceClient = new CrmService()
                {
                    ...
                    CrmAuthenticationTokenValue = token.CrmToken
                };

                switch (token.CrmToken.AuthenticationType)
                {
                    case AuthenticationType.AD:
                        ...
                        serviceClient.Credentials = new System.Net.NetworkCredential(authRequest.Username, Globals.UnwrapSecureString(authRequest.Password), authRequest.DomainName);
