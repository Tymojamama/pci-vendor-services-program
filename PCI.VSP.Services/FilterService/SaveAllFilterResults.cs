...
namespace PCI.VSP.Services
{
    public partial class FilterService
    {
        private void SaveAllFilterResults()
        {
            ...
            foreach (Filter.VendorProductFilterResult selectedFilterResult in _filterResultsSelected)
            {
                if (_filter.FilterPhase == Filter.FilterPhases.Phase1)
                {
                    SavePhase1ProjectVendorResult(selectedFilterResult);
                }
                else if (_filter.FilterPhase == Filter.FilterPhases.Phase2)
                {
                    SavePhase2ProjectVendorResult(selectedFilterResult);
                }
            }
        }
    }
